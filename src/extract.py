...
- Only transcribe dosing that is actually written. If a medication's dose, frequency, or duration is not written on the page, use null for that field. Do not supply a typical, default, or assumed course: for example, never write a duration such as "7 days" for a cream when no duration is printed. Expanding shorthand applies only to notation that is present; it never means filling in a value that is absent.
